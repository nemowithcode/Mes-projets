...
def page(text1: str, text2: str, fonction):
    ...
    def chiffrement(dechiffre):
        ...
        if dechiffre:
            ...
            entry3.insert(1.0, Chiffrement_cesar.dechiffrement(entry1.get(), entry2.get()))
        else:
            """ Effectue et afficher la réponse du chiffrement """
            entry3.delete(1.0, END)
            entry3.insert(1.0, Chiffrement_cesar.chiffrement(entry1.get(), int(entry2.get())))
...
window = Tk()
window.title("CHIFFREMENT DE CESAR")
window.geometry("1200x800")
window.config(bg="#24292E")
window.iconbitmap("Jules_cesar.ico")
